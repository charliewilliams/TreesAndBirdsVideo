...
public class Bird {
	...
	static private float sc = 3; //scale factor for the render of the boid
...
//		PApplet.println(pos);
	...
	void checkAvoidWalls() {
		...
		acc.add(PVector.mult(avoid(new PVector(pos.x, stage.y * 0.6667f, pos.z), true), wallAvoidWeight));
		acc.add(PVector.mult(avoid(new PVector(pos.x, 0, pos.z), true), wallAvoidWeight));
		acc.add(PVector.mult(avoid(new PVector(stage.x, pos.y, pos.z), true), wallAvoidWeight));
		acc.add(PVector.mult(avoid(new PVector(0, pos.y, pos.z), true), wallAvoidWeight));
		acc.add(PVector.mult(avoid(new PVector(pos.x, pos.y, 300), true), wallAvoidWeight));
		acc.add(PVector.mult(avoid(new PVector(pos.x, pos.y, 900), true), wallAvoidWeight));
	}
	...
	void render(PGraphics2D ps) {
	  ...
	  void render(PGraphics3D pg) {

		    pg.pushMatrix();
		    pg.translate(pos.x, pos.y, pos.z);
		    pg.rotateY((float) Math.atan2(-vel.z, vel.x));
		    pg.rotateZ((float) Math.asin(vel.y / vel.mag()));
		    pg.stroke(hue);
		    pg.noFill();
		    pg.noStroke();
		    pg.fill(hue, 100, 100);
